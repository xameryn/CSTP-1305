...
//                    S                F     
    ...
    sortedListMerge(newList) {
        let current1 = this.head;
        let current2 = newList.head;
        let currentListData = [];
        let newListData = [];

        while (current1) { // add current1 data to array
            newListData.push(current1.data);
            current1 = current1.next;
        }

        while (current2) { // add current2 data to array
            newListData.push(current2.data);
            current2 = current2.next;
        }

        let mergedData = currentListData.concat(newListData);

        mergedData.sort((a, b) => b - a);

        let mergedList = new LinkedList();
        mergedData.forEach(data => mergedList.prepend(data));
    ...
    deleteFromEnd(index) {
        ...
        while (current) { // add data to array
            listArray.push(current.data);
            current = current.next;
        }

        let deleteIndex = listArray.length - index; // get the index to delete
...
mergedList = list1.sortedListMerge(list2);

mergedList.printList();

mergedList.deleteFromEnd(1);
mergedList.deleteFromEnd(2);
